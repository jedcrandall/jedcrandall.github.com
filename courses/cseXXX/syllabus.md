...
- Course Name: CSE XXX, Internet Information Controls
...
This course will cover advanced topics, relevant research, and current trends
in the use of Network Intrusion Detection System (NIDS) systems on the Internet
for information controls at the scale of entire nations.

## Course Objectives

- Students will gain an understanding of how Network Intrusion Detection
  Systems (NIDSs) are applied by different nation states around the world for
  information controls.
- Students will gain an understanding of the social, ethical, and technical
  challenges inherent in measuring Internet censorship in different parts of
  the world.
- Students will gain an understanding of circumvention tools, such as Tor and
  VPNs, and the technical details of how they are implemented.
...
No prerequisite courses, but students are encouraged to know the basics of
cybersecurity (e.g., symmetric and public key cryptography) and networking
(e.g., IP routing) before taking the course.  We will review all prerequisite
knowledge, but briefly.

## Grading Policies, Assignments, and Required Materials

Half of your grade will be reading papers and participating in in-class
discussions.  This portion will be self-graded but you need to attend class or
have an excused absence to give yourself a grade.  Participation points will
add up to 300 points total.  There will be also three substantial homeworks
each totalling up to 100 points for 300 total.  Your grade is 50% based on
participation and 50% based on your homework, and will be your total points
divided into the 600 possible.  Grades are based on the following scale where x
is the percentage: 97.0 <= x <= 100.0 is an A+, 93.0 <= x < 97.0 is an A, 90.0
<= x < 93.0 is an A-, 87.0 <= x < 90.0 is an B+, 83.0 <= x < 87.0 is an B, 80.0
<= x < 83.0 is an B-, 77.0 <= x < 80.0 is an C+, 73.0 <= x < 77.0 is an C, 70.0
<= x < 73.0 is an C-, 60.0 <= x < 70.0 is D, and x < 60.0 is an E.
...
The course will be a combination of lectures, in-class discussion of research
papers, and homework assignments.  Attendance is required.
...
Students will be evaluated on their participation in reading and discussions,
and their performance on homework assignments.  There will not be any exams.
Attendance is not recorded but will be reflected in your attendance grade.
